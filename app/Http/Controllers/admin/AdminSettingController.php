...
// 
    ...
    public function editCustomFieldsName($id, $v = 1)
    {
        if ($v === 1) {
            $a = DB::table('wallets')->where('id', '=', $id)->select(['type', 'name'])->first();
            return $a === null ? null : '<input type="text" readonly style="height:40px;width:400px;" value="'.$a->type.' - '.$a->name.'"><input type="hidden" name="coin-name" value="'.$_GET['id'].'">';
        } else {
            $a = DB::table('market')
                ->join('wallets', 'market.wallet_from', '=', 'wallets.id')
                ->where('market.id', '=', $id)
                ->select(['wallets.type', 'wallets.name'])
                ->first();
            return $a === null ? null : '<input type="text" readonly style="height:40px;width:400px;" value="'.$a->type.' - '.$a->name.'"><input type="hidden" name="coin-name" value="'.$_GET['id'].'">';
        }
    }
