	...
	*/
			...
			case 'fee':
				...
				foreach ($markets as $market) {	
					...
					foreach($wallets as $wallet) {
						if ($market->wallet_from == $wallet->id) { $from = $wallet->type; }
						if ($market->wallet_to == $wallet->id) { $to = $wallet->type; }
					}
	...
	public function addNewCoinVote(){
		...
		if(isset($check_coinvote->id)){
			...
			}else return Redirect::to('admin/manage/coins-voting')->with('error', Lang::get('messages.not_found_btc_wallet') );			
		}
	} 
	...
	public function editUSer($user_id=''){
		...
		if(!isset($user->id)) return Redirect::to('admin/manage/users')->with( 'error', Lang::get('messages.user_not_exist') );
    ...
    public function editWallet($wallet_id=''){
		...
		if(!isset($wallet->id)) return Redirect::to('admin/manage/wallets')->with( 'error', Lang::get('messages.wallet_not_exist') );		
		$data['wallet'] = $wallet;
    ...
    public function addNewMarket(){ 
    	...
    	if($wallet_from == $wallet_to) return Redirect::to('admin/manage/markets')->with( 'error', Lang::get('messages.walletfrom_different_walletto') );
    ...
    public function addNewPost(){ 
    	...
    	}else return Redirect::to('admin/content/add-'.$post->type)->with( 'error', Lang::get('messages.fill_all_fields') );   	
    }
    ...
    public function doEditPost(){
    	...
    	}else return Redirect::to('admin/content/add-news')->with( 'error', Lang::get('messages.fill_all_fields')  ); 
    }
    ...
    public function formRestore(){
   	...
   	public function editLimitTrade($wallet_id=''){
		...
		if(!isset($limit_trade->id)) return Redirect::to('admin/setting/limit-trade')->with( 'error', Lang::get('messages.limit_trade_not_exist') );			
